...
class BaseTransform(object):
    ...
    def __call__(self, index, image):
        ...
        try:
            ...
            _, _, w, h = image.size()
        left = int(round((w - self.res[0]) / 2.))
        top = int(round((h - self.res[1]) / 2.))
        return TF.crop(image, top, left, self.res[0], self.res[1])
...
class RandomCrop(object):
    ...
    def __call__(self, index, image):
        ...
        w, h = image.size
        left = int(round((w - self.res[0]) * ws))
        top = int(round((h - self.res[1]) * hs))
        return TF.crop(image, top, left, self.res[0], self.res[1])
...
class RandomResizedCrop(object):
    ...
    def __call__(self, indice, image):
        new_image = []
        res_tar = [self.res[0]//4, self.res[1]//4] if image.size(1) > 5 else self.res  # View 1 or View 2?
